...
public class Associations {
    ...
    private HashMap<String, Relation> manyToManyAssociations = new HashMap<String, Relation>();
	...
	manyToManyAssociations.put(attribute, rc);
    }

    public void saveManyToManyRelations() {
	clearKeysPairs();
	for (String attribute : manyToManyAssociations.keySet()) {
	    Jointure jointure = manyToManyAssociations.get(attribute)
		    .getJointure();
	...
	for (String attr : manyToManyAssociations.keySet()) {
		...
		Jointure jointure = manyToManyAssociations.get(attr)
			.getJointure();
		Set<Jointure> jointures = (Set<Jointure>) Finder.joinFind(
			object.getId(), jointure);
		ParameterizedType type = (ParameterizedType) object.getClass()
			.getDeclaredField(attr).getGenericType();
	...
	for (String attr : hasOneAssociations.keySet()) {
	    try {
		object.getAssociations().getHasOneAssociations().get(attr)
			.setFkValue(rs.getInt(attr + "_id"));
		Field field = object.getClass().getDeclaredField(attr);
		Hermes obj = (Hermes) field.getType().newInstance();
		obj = Finder.find(hasOneAssociations.get(attr).getFkValue(),
			obj.getClass());
		field.setAccessible(true);
    ...
    public HashMap<String, Relation> getManyToManyAsociations() {
    ...
    public void setHasManyRelationsShip(
	    HashMap<String, Relation> hasManyRelationsShip) {
